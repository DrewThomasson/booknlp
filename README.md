...
### HTML Visualization (`.book.html`)

This file provides a visual representation of the book with annotations for characters, quotes, and other features that can be viewed in a web browser.

## Analysis Tools

This repository includes several Python scripts for analyzing BookNLP output files and generating visualizations. These tools help you understand character relationships, dialogue patterns, and overall narrative structure.

### Installation

The analysis tools require additional Python packages. Install them using:

```bash
pip install pandas matplotlib networkx jinja2
```

### Usage

The main analysis tool is `booknlp_analyzer.py`, which provides several commands for different types of analysis:

1. Generate a complete HTML dashboard:
```bash
python booknlp_analyzer.py dashboard output/pride_improved
```

2. Generate character statistics and visualizations:
```bash
python booknlp_analyzer.py character-stats output/pride_improved
```

3. Create a character network visualization:
```bash
python booknlp_analyzer.py character-network output/pride_improved
```

4. Analyze all books in a directory:
```bash
...
The analysis tools provide the following features:

1. **Character Statistics**
   - Top characters by number of mentions
   - Character type distribution (PER, LOC, ORG)
   - Quote distribution among characters
   - Visualizations of character mentions and dialogue patterns

2. **Character Network Analysis**
   - Interactive network visualization of character relationships
   - Centrality analysis to identify key characters
   - Edge weights based on interaction frequency
   - Minimum mention threshold to filter minor characters

3. **Interactive Dashboard**
   - Comprehensive HTML dashboard with all analyses
   - Interactive visualizations
   - Sample quotes from main characters
   - Summary statistics and metrics

### Output Examples

The analysis tools generate various output files:

1. **HTML Dashboard** (`{book_name}_dashboard.html`)
   - Complete analysis in an interactive web interface
   - Character statistics and visualizations
   - Network analysis and centrality metrics
   - Sample quotes and dialogue patterns

2. **Visualization Files** (in `figures/` directory)
   - `{book_name}_character_mentions.png`: Bar chart of character mentions
   - `{book_name}_quote_distribution.png`: Distribution of dialogue
   - `{book_name}_character_network.png`: Network visualization

### Example Workflow

1. First, process your text file with BookNLP:
```bash
./run_booknlp.py pride.txt --output-dir output/pride_improved
```

2. Then, generate a complete analysis dashboard:
```bash
python booknlp_analyzer.py dashboard output/pride_improved
```
...
```bash
open output/pride_improved_dashboard.html
```

The dashboard will provide an interactive view of all analyses, including character relationships, dialogue patterns, and narrative structure.
